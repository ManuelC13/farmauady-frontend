        ...
        <table className="w-full text-left">
          ...
          <thead className="sticky top-0 z-10 bg-lightBlue">
            <tr className="border-b border-gray-300">
              ...
              <th className="py-4 px-6 text-xs font-semibold text-titleBlue tracking-wider uppercase">Caducidad</th>
              <th className="py-4 pl-11 pr-4 text-xs font-semibold text-titleBlue tracking-wider uppercase">Estado</th>
              <th className="py-4 pl-8 pr-5 text-xs font-semibold text-titleBlue tracking-wider uppercase">Acciones</th>
            </tr>
          </thead>
            ...
            {products.length === 0 ? (
              ...
              products.map((product) => (
                <tr key={product.id_product} className="border-b border-gray-300 hover:bg-secondary/10 transition">
                  ...
                  {/* Categoría */}
                  <td className="py-4 pl-6 pr-2 text-sm text-gray-800">
                    {product.category?.name}
                  </td>

                  {/* Precio */}
                  <td className="py-4 pl-3 pr-6 text-sm text-gray-800">
                    ${Number(product.sale_price).toFixed(2)}
                  </td>

                  {/* Existencias */}
                  <td className="py-4 px-6 text-center">
                    <span className={`inline-flex items-center px-4 py-1 rounded-full text-sm font-semibold ${getStockStyle(product.stock, product.minimum_stock)}`}>
                      {product.stock} u.
                    </span>
                  </td>
